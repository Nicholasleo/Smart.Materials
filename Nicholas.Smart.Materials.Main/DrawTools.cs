...
namespace Nicholas.Smart.Materials.Main
{
    ...
    class DrawTools
    {
        ...
        public void Draw(MouseEventArgs e, string sType)
        {
            if (startDraw)
            {
                ...
                switch (sType)
                {
                    ...
                    case "Word":
                        //newgraphics.DrawString(new SolidBrush(drawColor), rectStartPointF.X, rectStartPointF.Y, width, heigth);
                        break;
                }
        ...
        public void DrawWord(MouseEventArgs e)
        {
            if (startDraw)
            {
                ...
                PointF currentPointF = new PointF(e.X, e.Y);
                newgraphics.DrawLine(p, startPointF, currentPointF);
                newgraphics.DrawString("111", f, drawBrush, startPointF);
                startPointF = currentPointF;
                newgraphics.Dispose();
                DrawTools_Graphics.DrawImage(finishingImg, 0, 0);
            }
